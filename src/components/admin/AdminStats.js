...
const AdminStats = () => {
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'}>
      <h2 className={`text-2xl font-extrabold text-primary-900 mb-7 tracking-tight ${isRTL ? 'text-right' : 'text-left'}`}>
        {t('system_overview')}
      </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 md:gap-8 mb-8">
        {mainStats.map((stat, index) => {
          ...
          return (
            ...
            >
              <div className={`flex items-center gap-3 sm:gap-1 justify-between ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                <div
                  className={`
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 md:gap-8">
        ...
        <div className="xl:col-span-2 order-2 xl:order-1">
          <div className="bg-white rounded-2xl shadow-lg p-5 md:p-7 h-full flex flex-col">
            <h3 className={`text-lg md:text-xl font-semibold text-primary-900 mb-4 flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
              ...
              <div className="space-y-4">
                {stats.recentAppointments && stats.recentAppointments.length > 0 ? (
                  stats.recentAppointments.map((appointment) => (
                    <div
                      key={appointment._id}
                      className={`flex flex-col md:flex-row items-start md:items-center justify-between gap-4 md:gap-0 p-3 border border-gray-100 hover:border-primary-200 rounded-xl transition ${isRTL ? 'text-right' : 'text-left'}`}
                    >
                      ...
                      <div className={isRTL ? "text-left" : "text-right"}>
                        <p className="text-sm font-bold text-primary-700 tabular-nums">
                          {new Date(appointment.date).toLocaleDateString(i18n.language === 'ar' ? 'ar-EG' : 'en-US')}
                        </p>
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-400">{t('no_recent_appointments')}</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="order-1 xl:order-2 flex flex-col gap-5">
          <div className="bg-white rounded-2xl shadow-lg p-5 md:p-7">
            <h3 className={`text-lg md:text-xl font-semibold text-primary-800 mb-4 ${isRTL ? 'text-right' : 'text-left'}`}>
              {t('quick_actions')}
            </h3>
            <div className="flex flex-col gap-3">
              <button 
                onClick={() => window.location.hash = '#users'}
                className={`w-full flex items-center justify-between px-4 py-3 border border-gray-200 rounded-lg bg-gradient-to-r from-white to-primary-50 hover:from-primary-50 hover:to-primary-100 transition group ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}
              >
                <span className="text-gray-800 font-medium group-hover:text-primary-600 transition">
                  {t('user_management')}
                </span>
              ...
              <button 
                ...
                className={`w-full flex items-center justify-between px-4 py-3 border border-gray-200 rounded-lg bg-gradient-to-r from-white to-green-50 hover:from-green-50 hover:to-green-100 transition group ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}
              >
                <span className="text-gray-800 font-medium group-hover:text-green-700 transition">
              ...
              <button
                className={`w-full flex items-center justify-between px-4 py-3 border border-gray-200 rounded-lg bg-gradient-to-r from-white to-orange-50 hover:from-orange-50 hover:to-orange-100 transition group ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}
              >
                <span className="text-gray-800 font-medium group-hover:text-orange-600 transition">
                  {t('system_reports')}
                </span>
          ...
          <div className="bg-gradient-to-tr from-primary-50 to-white border border-primary-100 rounded-2xl shadow p-5 md:p-6 mt-2">
            <h3 className={`text-lg font-semibold text-primary-900 mb-3 flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
              <ChartBarIcon className={`w-5 h-5 text-primary-500 ${isRTL ? 'ml-1' : 'mr-1'}`} />
              {t('system_info')}
            </h3>
            <div className={`space-y-2 text-sm text-primary-900 font-medium ${isRTL ? 'text-right' : 'text-left'}`}>
